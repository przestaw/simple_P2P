...
namespace simpleP2P {
RequestServer::RequestServer(boost::asio::io_service &_io_service, Uint16 port, FileManager &fm, LoggingModule &lm)
  ...
  if (!error) {
    ...
    try {
      ...
      logging_module.add_log_line(
          "RequestServer: exception caught from RequestWorker::start()! Deleting RequestWorker object",
          std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));

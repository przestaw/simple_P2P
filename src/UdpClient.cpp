...
simpleP2P::UdpClient::UdpClient(io_service &io_service,
  ...
  timer.async_wait(boost::bind(&UdpClient::fire_beacon,
                               this));
}
...
void simpleP2P::UdpClient::send(const std::vector<Uint8> &packet) {
...
void simpleP2P::UdpClient::transmit() {
  socket_.async_send_to(
      boost::asio::buffer(&tx_queue_.front()[0], tx_queue_.front().size()),
      endpoint_,
      boost::bind(&UdpClient::write_callback,
                  boost::weak_ptr<UdpClient>(shared_from_this()),
                  boost::asio::placeholders::error,
                  boost::asio::placeholders::bytes_transferred));
}

void simpleP2P::UdpClient::write_handler(boost::system::error_code const &error,
...
void simpleP2P::UdpClient::write_callback(boost::weak_ptr<UdpClient> ptr,
                                          boost::system::error_code const &error,
                                          size_t bytes_transferred) {
  boost::shared_ptr<UdpClient> pointer(ptr.lock());
  if (pointer && (boost::asio::error::operation_aborted != error))
    pointer->write_handler(error, bytes_transferred);
}

void simpleP2P::UdpClient::revoke_file(simpleP2P::Resource resource) {
...
void simpleP2P::UdpClient::fire_beacon() {

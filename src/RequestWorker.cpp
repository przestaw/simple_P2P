...
void RequestWorker::start() {
  ...
  if (send_data == nullptr) {
    logging_module.add_log_line(
        "RequestWorker::start(): allocating memory for a segment to be sent "
        "FAILED! (malloc returned nullptr) Throwing 			"
        "								"
        "						   exception...",
  ...
  logging_module.add_log_line(
      "RequestWorker successfully started",
      std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));

  _socket.async_read_some(
      boost::asio::buffer(recv_data, MAX_RECV_DATA_LENGHT),
...
void RequestWorker::handle_read(const boost::system::error_code &error,
  ...
  if (!error) {
    ...
    if (command == REQ_SEGMENT) {
      Uint64 file_size;
      std::memcpy(&file_size, recv_data + 1 + FILE_NAME_LENGHT,
                  FILE_SIZE_LENGHT);
      ...
      std::string logmsg = "RequestWorker: request for segment " +
                           ...
                           "' received";
      logging_module.add_log_line(logmsg,
      ...
      // Check if the requested segment is the last segment of the file (and its
      // size != SEGMENT_SIZE). If this is the case, calculate its size.
      if (segment == static_cast<Uint16>(nsegments)) {
        requested_segment_size =
            file_size - static_cast<Uint16>(nsegments) * SEGMENT_SIZE;
      } else {
      ...
      if (!file_manager.get_segment(file_name, segment, send_data,
                                    requested_segment_size)) {
        logging_module.add_log_line(
            "RequestWorker: getting the requested segment from file manager "
            "FAILED. Deleting RequestWorker object",
            std::chrono::system_clock::to_time_t(
                std::chrono::system_clock::now()));
        delete this;
      } else {
        logmsg = "RequestWorker: sending segment " + std::to_string(segment) +
                 " of file: '" + file_name + "'...";
        logging_module.add_log_line(logmsg,
                                    std::chrono::system_clock::to_time_t(
                                        std::chrono::system_clock::now()));
        ...
        boost::asio::async_write(
            _socket, boost::asio::buffer(send_data, std::size_t(SEGMENT_SIZE)),
            boost::bind(&RequestWorker::handle_write, this,
                        boost::asio::placeholders::error));
      }
      //}
    } else if (command == QUIT_CONN) {
      logging_module.add_log_line(
          "RequestWorker: client requested a connection quit. Deleting "
          "RequestWorker object",
          std::chrono::system_clock::to_time_t(
              std::chrono::system_clock::now()));
    ...
    logging_module.add_log_line(
        "RequestWorker: ERROR reading data from the client! Deleting "
        "RequestWorker object",
        std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));
    delete this;
  }
...
void RequestWorker::handle_write(const boost::system::error_code &error) {
  if (!error) {
    logging_module.add_log_line(
        "RequestWorker: segment successfully sent",
        std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));
    ...
    logging_module.add_log_line(
        "RequestWorker: ERROR sending a segment! Deleting RequestWorker object",
        std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));

    ...
    void RequestWorker::handle_read(const boost::system::error_code &error,
        ...
        if (!error) {
            ...
            if (command == REQ_SEGMENT) {
                ...
                std::string logmsg =
                        "RequestWorker: request for segment " + std::to_string(segment) + " of file: '" + file_name +
                        "' received";
                ...
                logging_module.add_log_line(
                        "RequestWorker: client requested a connection quit. Deleting RequestWorker object",
                        std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));
            ...
            logging_module.add_log_line(
                    "RequestWorker: ERROR reading data from the client! Deleting RequestWorker object",
                    std::chrono::system_clock::to_time_t(std::chrono::system_clock::now()));
            delete this;
        }
    }

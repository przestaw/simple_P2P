...
namespace simpleP2P {
Resource_Database::Resource_Database(Host localhost)
    : my_host(std::move(localhost)) {}
...
void Resource_Database::add_file(const Resource &res, const Host &host) {
  std::unique_lock lock(database_mutex);
  auto res_i = std::find_if(resources.begin(), resources.end(),
                            [&res](Resource &it) { return res == it; });
  auto host_i = std::find_if(hosts.begin(), hosts.end(),
                             [&host](Host &it) { return host == it; });
...
std::vector<Host *>
Resource_Database::who_has_file(std::vector<Int8> resource_header) {
...
std::vector<Host *> Resource_Database::who_has_file(const Resource &res) {
  ...
  auto resource = std::find(resources.begin(), resources.end(), res);
...
std::vector<Int8> Resource_Database::generate_database_header() {
  ...
  auto host = std::find(hosts.begin(), hosts.end(), my_host);

...
namespace simpleP2P
{
CLI::CLI(Resource_Database &res_db_, Logging_Module &Logger_, boost::asio::io_service &io_service_, FileManager &fm_, Host &localhost_, Printer &printer_) : res_db(res_db_), Logger(Logger_), io_service(io_service_), fm(fm_), localhost(localhost_), printer(printer_)
{
    CLICommands = {
        ...
        CLICommand("revoke", "\"revoke name_of_file\" - invalidates a resource", [this](std::string name_of_file) {
